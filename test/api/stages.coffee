...
describe 'Stages', ->
  ...
  entity = name: 'stage', plural: 'stages'
  ...
  before (done)->
    ...
    # Starting the server
    api.init ->
      api.models.Report.create {title: 'Some report'}, (err, doc)->
        report = doc
        api.models.Stage.create {
          title: 'Some stage',
          visual_type: 'grid',
          report_id: report._id}, (err, stage)->
            if stage
              api.models.Stage.create {
                title: 'Another stage',
                visual_type: 'matrix',
                report_id: report._id}, (err, stage)->
                  if stage then done() else throw err
            else
              throw err
